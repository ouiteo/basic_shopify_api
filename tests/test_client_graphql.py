...
@pytest.mark.usefixtures("local_server")
@async_local_server_session
async def test_is_bulk_job_running(local_server, mock_graphql_response: AsyncMock, job_status: str, expected: bool) -> None:
    async with AsyncClient(*generate_opts_and_sess()) as c:
...
@pytest.mark.usefixtures("local_server")
@async_local_server_session
async def test_run_bulk_operation_query(local_server, mock_graphql_response: AsyncMock, monkeypatch: pytest.MonkeyPatch, expected_type: type, wait: bool) -> None:
    async with AsyncClient(*generate_opts_and_sess()) as c:
...
@pytest.mark.usefixtures("local_server")
@async_local_server_session
async def test_run_bulk_operation_mutation(local_server, mock_graphql_response: AsyncMock, monkeypatch: pytest.MonkeyPatch, httpx_mock: HTTPXMock, expected_type: type, wait: bool) -> None:
    async with AsyncClient(*generate_opts_and_sess()) as c:
        mock_graphql_response(f"create_and_submit_mutation.json")
